...
def main():
    ...
    for query, category, domain_filter in searches:
        ...
        for r in results:
            ...
            # For X/Twitter: enrich summary with full tweet text via oEmbed (Exa only returns snippet)
            if domain_filter == "x":
                source_tag = f"X: @{author}" if author else f"X: {raw_domain}"
                # Try oEmbed for full tweet text (oEmbed is faster and returns complete content)
                ...
                try:
                    ...
                    pass  # Keep Exa's text if oEmbed fails
                stats["x"] += 1
            elif domain_filter == "linkedin":
