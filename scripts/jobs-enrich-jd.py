...
# LinkedIn auth: NO cookies/Voyager -- use JobSpy tls-client approach only
...
def fetch_linkedin_jobspy(job_id, title_hint=""):
    """Fetch LinkedIn JD using JobSpy's approach: tls-client with Chrome TLS fingerprint.
    Hits LinkedIn's public /jobs/view/{job_id} endpoint -- no cookies or login needed.
    Same mechanism as python-jobspy 1.1.82 with linkedin_fetch_description=True."""
    # Check cache first
...
def run():
    ...
    # Prioritize: no JD yet first (new jobs), then by keyword_score descending
    no_jd = [j for j in jobs if not j.get("jd_text") or len(j.get("jd_text", "")) <= 100]
    has_jd = [j for j in jobs if j.get("jd_text") and len(j.get("jd_text", "")) > 100]
    no_jd_sorted = sorted(no_jd, key=lambda j: j.get("keyword_score", 0), reverse=True)
    has_jd_sorted = sorted(has_jd, key=lambda j: j.get("keyword_score", 0), reverse=True)
    sorted_jobs = no_jd_sorted + has_jd_sorted
    top_jobs = sorted_jobs[:MAX_FETCH]
    print(f"  Enrichment queue: {len(no_jd)} need JD, {len(has_jd)} already have JD — processing top {len(top_jobs)}")
    ...
    def enrich_one(job, idx):
        ...
        # LinkedIn: tls_client only (JobSpy approach — no cookies/Voyager)
        if "linkedin.com" in url:
