...
# ─────────────────────────────────────────────
# STEP 5: CRON DELIVERY VERIFICATION (Notion)
# ─────────────────────────────────────────────

def check_briefing_notion():
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    result = {"today": today, "exists": False}
    ...
    try:
        ...
        if exists:
            add_finding("briefing", "ok", f"Notion briefing page for {today}: exists")
        else:
            add_finding("briefing", "alert", f"Notion briefing page for {today}: MISSING - cron may have failed")
            needs_attention.append(f"Morning briefing Notion page missing for {today} - partial delivery failure")

    except Exception as e:
        add_finding("briefing", "warn", f"Notion briefing check failed: {e}")
        result["error"] = str(e)
