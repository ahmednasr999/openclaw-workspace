...
def twitter_tweet(url_or_id):
    """
    Get tweet details. Pass full URL or tweet ID.
    Tries ScrapeCreators API first (if key available), falls back to oEmbed (free, no key needed).
    """
    ...
    if not url_or_id.startswith("http"):
        ...
        url = url_or_id.replace("twitter.com", "x.com")
    # Try ScrapeCreators first (if API key set)
    scrape_key = os.environ.get("SCRAPECREATORS_API_KEY") or \
                 os.environ.get("SCRAPECREATORS_KEY")
    if scrape_key:
        result = call("twitter/tweet", {"url": url})
        if result and not result.get("error"):
            return result
    # Fall back to free oEmbed
    try:
        oembed_url = f"https://publish.twitter.com/oembed?url={urllib.parse.quote(url)}&dnt=true"
        req = urllib.request.Request(oembed_url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=10) as resp:
            oembed = json.loads(resp.read())
        text = re.sub(r"<[^>]+>", "", oembed.get("html", "")).strip()
        return {
            "url": url,
            ...
            "date": re.search(r'[A-Z][a-z]+ \d+, \d{4}', oembed.get("html", "") or "").group(0) if re.search(r'[A-Z][a-z]+ \d+, \d{4}', oembed.get("html", "") or "") else "",
            "source": "oembed",
        }
