...
def log_source_run(
    ...
    unique_count: int = 0,
    db_registered: int = 0,
    countries_json: str = None,
    titles_json: str = None,
    duration_ms: int = 0,
    errors: int = 0,
    status: str = "success",
):
    """Log a source run to source_runs table for SAYYAD reporting."""
    if not ENABLED:
        return
    try:
        conn = _get_conn()
        conn.execute("""
            INSERT INTO source_runs
                (run_date, source, raw_count, unique_count, db_registered,
                 countries_json, titles_json, duration_ms, errors, status, created_at)
            VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (source, raw_count, unique_count, db_registered,
              countries_json, titles_json, duration_ms, errors, status, _now_iso()))
        conn.commit()
        conn.close()
    except Exception as e:
        log.error("log_source_run failed for %s: %s", source, e)
