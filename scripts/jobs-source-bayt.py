#!/usr/bin/env python3
"""
jobs-source-bayt.py — Bayt job source adapter using python-jobspy.

Uses python-jobspy library to scrape Bayt jobs.
Searches 22 titles x ALL 6 GCC countries (Bayt is GCC-native).
72h lookback window.

Output: data/jobs-raw/bayt.json
"""
...
# Configuration
AGENT_NAME = "jobs-source-bayt"
OUTPUT_FILE = JOBS_RAW_DIR / "bayt.json"
RESULTS_PER_SEARCH = 10
HOURS_OLD = 72  # 3 days for Bayt
RATE_LIMIT_DELAY = 1.5


def search_bayt_jobs(title: str, country: str) -> list[dict]:
    """Search Bayt using jobspy. Returns list of job dicts."""
    try:
...
def run_bayt_scanner(result: AgentResult):
    """Main scanner logic."""
    ...
    if is_dry_run():
        ...
        print("\n--- ALL GCC Countries (Bayt is GCC-native) ---")
        ...
        result.set_kpi({
            ...
            "countries": len(GCC_COUNTRIES),
            "titles": len(ALL_TITLES),
        })
        return
    
    # Build search plan - ALL 6 GCC countries
    searches = []
    for country in PRIORITY_COUNTRIES:
        for title in ALL_TITLES:
    ...
    print(f"Search plan: {len(searches)} queries (Bayt - all GCC)")
    ...
    for title, country in searches:
        ...
        jobs = search_bayt_jobs(title, country)
