...
def check_auto_drafter():
    """Auto-Drafter (9 AM): drafts being created?"""
    # Check for recent Draft entries in Content Calendar
    drafts, err = query_db(CAL_DB, {"property": "Status", "select": {"equals": "Draft"}})
    if err: return {"status": "fail", "message": err}
    if len(drafts) == 0:
        return {"status": "warn", "message": "No drafts in queue (drafter may not have run)"}
