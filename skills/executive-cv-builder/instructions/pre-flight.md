...
1. **Model check:** Verify CV generation is running on **openai-codex/gpt-5.5**. If not, STOP and switch to GPT-5.5 before generating or scoring. Never generate a CV on any other model unless Ahmed explicitly changes the rule in the current task. Zero exceptions.
