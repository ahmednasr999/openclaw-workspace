...
pptx.subject = 'Notion branded showcase';
pptx.title = 'Notion Branded Showcase';
pptx.lang = 'en-US';
pptx.theme = {
  headFontFace: 'NotionInter',
  bodyFontFace: 'NotionInter',
  lang: 'en-US',
};

const OUT = path.join(__dirname, 'exports', 'notion-branded-showcase.pptx');
const TOKENS = {
  bgWarm: '191918',
  bgWhite: '000000',
  surface: '000000',
...
function addPageBadge(slide, n) {
  slide.addText(String(n), {
    ...
    fontFace: 'NotionInter', fontSize: 10, bold: true,
...
function addEyebrow(slide, text, x, y, w) {
  slide.addText(text, {
    x, y, w, h: 0.22,
    fontFace: 'NotionInter', fontSize: 9, bold: true,
...
function addRiskNote(slide, text) {
  slide.addText(text, {
    ...
    fontFace: 'NotionInter', fontSize: 8.5, color: TOKENS.textSoft, margin: 0,
  });
}
...
function addVisualPlaceholder(slide, label, x, y, w, h, fill = TOKENS.surface) {
  ...
  slide.addText(label, {
    ...
    fontFace: 'NotionInter', fontSize: 14, color: TOKENS.textSoft,
...
function coverSlide() {
  ...
  addEyebrow(slide, 'notion showcase · cover', 0.7, 0.55, 3.2);
  slide.addText('Notion Branded Showcase', {
    x: 0.7, y: 1.1, w: 6.7, h: 1.4,
    fontFace: 'NotionInter', fontSize: 31, bold: true,
  ...
  slide.addText('A concrete starter deck for the redesigned slides skill, showing how planning patterns map into editable PPTX composition.', {
    ...
    fontFace: 'NotionInter', fontSize: 16, color: TOKENS.textSecondary,
    margin: 0,
  });
  slide.addText('Goal: make the default output feel designed before a human touches it.', {
    x: 0.72, y: 4.2, w: 5.5, h: 0.45,
    fontFace: 'NotionInter', fontSize: 11, color: TOKENS.textSoft, margin: 0,
  });
...
function sectionDividerSlide() {
  const slide = pptx.addSlide();
  slide.background = { color: TOKENS.bgWarm };
  addEyebrow(slide, 'notion showcase · section-divider', 0.8, 0.75, 3.6);
  slide.addText('02', {
    x: 0.78, y: 1.35, w: 1.6, h: 1.15,
    fontFace: 'NotionInter', fontSize: 34, bold: true,
    color: TOKENS.accent, margin: 0,
  });
  slide.addText('Pattern logic', {
    x: 0.78, y: 2.9, w: 6.0, h: 0.8,
    fontFace: 'NotionInter', fontSize: 26, bold: true,
    color: TOKENS.textPrimary, margin: 0,
  });
  slide.addText('A section divider should feel like a chapter page, not a normal content slide with a bigger title.', {
    ...
    fontFace: 'NotionInter', fontSize: 13, color: TOKENS.textSecondary, margin: 0,
  });
...
function agendaSlide() {
  ...
  addEyebrow(slide, 'notion showcase · agenda', 0.7, 0.45, 3.2);
  slide.addText('Agenda', {
    x: 0.7, y: 0.9, w: 3.8, h: 0.45,
    fontFace: 'NotionInter', fontSize: 24, bold: true, color: TOKENS.textPrimary, margin: 0,
  });
  slide.addText('Six examples showing how pattern-aware planning should shape the default deck stub.', {
    x: 0.7, y: 1.5, w: 5.7, h: 0.45,
    fontFace: 'NotionInter', fontSize: 11, color: TOKENS.textSecondary, margin: 0,
  });
  ...
  items.forEach((item, idx) => {
    slide.addText(String(idx + 1).padStart(2, '0'), {
      ...
      fontFace: 'NotionInter', fontSize: 14, bold: true, color: TOKENS.accent, margin: 0,
    });
    slide.addText(item, {
      x: 1.55, y: y - 0.02, w: 4.6, h: 0.3,
      fontFace: 'NotionInter', fontSize: 15, color: TOKENS.textPrimary, margin: 0,
    });
    y += 0.66;
  });
...
function thesisSummarySlide() {
  const slide = pptx.addSlide();
  slide.background = { color: TOKENS.bgWhite };
  addEyebrow(slide, 'notion showcase · thesis-summary', 0.7, 0.45, 3.9);
  slide.addText('Thesis summary', {
    x: 0.7, y: 0.9, w: 4.6, h: 0.45,
    fontFace: 'NotionInter', fontSize: 23, bold: true, color: TOKENS.textPrimary, margin: 0,
  });
  slide.addText('The redesign should move slide quality from prompt luck to a constrained editorial system.', {
    ...
    fontFace: 'NotionInter', fontSize: 18, color: TOKENS.textPrimary, margin: 0,
  });
  ...
  supports.forEach((text) => {
    slide.addText(text, {
      ...
      fontFace: 'NotionInter', fontSize: 12, color: TOKENS.textSecondary, margin: 0,
    });
    y += 0.85;
  });
...
function metricSlide() {
  const slide = pptx.addSlide();
  slide.background = { color: TOKENS.bgWhite };
  addEyebrow(slide, 'notion showcase · metric', 0.7, 0.45, 3.2);
  slide.addText('Key KPI', {
    x: 0.7, y: 0.9, w: 4.0, h: 0.45,
    fontFace: 'NotionInter', fontSize: 22, bold: true, color: TOKENS.textPrimary, margin: 0,
  });
  slide.addText('72% automation coverage', {
    x: 0.7, y: 1.8, w: 5.8, h: 1.2,
    fontFace: 'NotionInter', fontSize: 30, bold: true, color: TOKENS.accent, margin: 0,
  });
  slide.addText('Up from 31% baseline in nine months, enough to justify scaling the operating model into two additional functions.', {
    ...
    fontFace: 'NotionInter', fontSize: 14, color: TOKENS.textSecondary, margin: 0,
  });
  slide.addShape(pptx.ShapeType.roundRect, {
...
function comparisonSlide() {
  const slide = pptx.addSlide();
  slide.background = { color: TOKENS.bgWhite };
  addEyebrow(slide, 'notion showcase · comparison', 0.7, 0.45, 3.5);
  ...
  slide.addText('Recommended lane should dominate when symmetry is not truthful.', {
    ...
    fontFace: 'NotionInter', fontSize: 11.5, color: TOKENS.textSecondary, margin: 0,
  });
  ...
  slide.addText('Status quo', { x: 1.0, y: 2.52, w: 2.3, h: 0.3, fontFace: 'NotionInter', fontSize: 17, bold: true, color: TOKENS.textPrimary, margin: 0 });
  slide.addText('Recommended lane', { x: 5.25, y: 2.55, w: 3.1, h: 0.3, fontFace: 'NotionInter', fontSize: 18, bold: true, color: TOKENS.textPrimary, margin: 0 });
  slide.addText('Preferred option gets more area, stronger stroke, and cleaner evidence hierarchy.', { x: 5.22, y: 3.15, w: 3.8, h: 1.0, fontFace: 'NotionInter', fontSize: 13, color: TOKENS.textSecondary, margin: 0 });
...
function twoColumnExplainerSlide() {
  const slide = pptx.addSlide();
  slide.background = { color: TOKENS.bgWhite };
  addEyebrow(slide, 'notion showcase · 2-column-explainer', 0.7, 0.45, 4.3);
  ...
  slide.addText('One side should explain. The other side should support. If both sides shout equally, this should probably be a comparison slide instead.', {
    ...
    fontFace: 'NotionInter', fontSize: 16, color: TOKENS.textPrimary, margin: 0,
  });
  ...
  slide.addText('support block', {
    ...
    fontFace: 'NotionInter', fontSize: 16, bold: true, color: TOKENS.textPrimary, margin: 0,
  });
...
function timelineSlide() {
  const slide = pptx.addSlide();
  slide.background = { color: TOKENS.bgWhite };
  addEyebrow(slide, 'notion showcase · process-timeline', 0.7, 0.45, 4.0);
  ...
  xs.forEach((x, idx) => {
    ...
    slide.addText(`Phase ${idx + 1}`, { x: x - 0.08, y: 2.66, w: 1.0, h: 0.22, fontFace: 'NotionInter', fontSize: 11, bold: true, color: idx === 1 ? TOKENS.accent : TOKENS.textPrimary, margin: 0 });
  });
  slide.addText('Current priority phase', { x: 3.0, y: 4.0, w: 2.3, h: 0.5, fontFace: 'NotionInter', fontSize: 12, color: TOKENS.textSecondary, margin: 0, align: 'center' });
...
function closingSlide() {
  ...
  slide.addText('The redesign matters when planning intent, pattern logic, and editable output all line up in the first generated draft.', {
    ...
    fontFace: 'NotionInter', fontSize: 14, color: TOKENS.textSecondary,
  ...
  slide.addText('next move: build more examples', {
    ...
    fontFace: 'NotionInter', fontSize: 11, bold: true, color: TOKENS.accent,
