...
Produce a verified coding closeout with tests/checks, Codex review findings, accepted/rejected decisions, and remaining risk.

## Operating rule

Review is evidence, not authority. Inspect source-of-truth repo state first, run focused checks, run Codex review on the actual diff, inspect every finding manually, fix only real issues, and verify again before closeout.

## Tool ladder

1. Inspect repo state and the actual diff.
2. Run the smallest meaningful tests/checks for the edited area.
3. Run Codex review using the local approved CLI path when available.
...
- `references/codex-review-command.md` - command patterns and safe fallbacks.
...
- Codex review was run, or unavailable status is stated.
