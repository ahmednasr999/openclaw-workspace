...
class AutoreviewHardeningTests(unittest.TestCase):
    ...
    def test_secret_detector_allows_chained_lookup_references(self) -> None:
        ...
        mapping_lookup = (
            "to"
            + "ken = tokens.get('access_"
            + "token')"
        )

        self.assertFalse(self.helper["secret_text_risk"](lookup))
        self.assertFalse(self.helper["secret_text_risk"](mapping_lookup))
        self.assertFalse(
            self.helper["secret_text_risk"](
                "to"
        ...
        self.assertTrue(
            self.helper["secret_text_risk"](
                mapping_lookup + " or 'ordinary-hardcoded-value-12345'"
            )
        )
        self.assertTrue(
            self.helper["secret_text_risk"](
                mapping_lookup[:-1]
                + ", 'ordinary-hardcoded-value-12345')"
            )
        )
        self.assertTrue(
            self.helper["secret_text_risk"](
