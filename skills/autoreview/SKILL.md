...
Codex review is the default when no engine is set. It uses `gpt-5.6-sol` with `high` reasoning by default. If Sol is unavailable, the helper fails visibly and does not substitute another model. Claude review is optional and uses `claude-fable-5` by default.
...
- Never switch or override the requested review engine/model. Sol access, capacity, rate-limit, and unrelated failures must fail visibly with the same engine/model. Claude fallback chains remain opt-in and must be explicitly configured.
...
| ------------------- | -------------------------------------------------- | ----------------------------------------------------- |
| **codex** (default) | `gpt-5.6-sol`                                       | NASR review default; no automatic model fallback     |
...
| ------------------- | -------------------------- | ---------------------------------------------------------------------------- | ----------------------------- | ---------------------------------------------------------- |
| **codex** (default) | `codex --model X exec ...` | `gpt-5.6-sol`; fail visibly if unavailable                                     | `-c model_reasoning_effort=Y` | `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max` |
